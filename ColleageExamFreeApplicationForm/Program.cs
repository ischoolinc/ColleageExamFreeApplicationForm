...
namespace ColleageExamFreeApplicationForm
{
    public class Program
    {
        ...
        {   // [ischoolkingdom] Vicky更動，原107年五專(優先)免試入學相關報表，移除"107年"字樣
            ...
            item1["報表"]["成績相關報表"]["五專免試入學相關報表"]["北中南五專(優先)免試入學集體報名表"].Enable = false;
            item1["報表"]["成績相關報表"]["五專免試入學相關報表"]["北中南五專(優先)免試入學集體報名表"].Click += delegate
            {
                Report_priority report = new Report_priority();
                report.ShowDialog();
            };
            item1["報表"]["成績相關報表"]["五專免試入學相關報表"]["南區五專聯合免試入學集體報名表"].Enable = false;
            item1["報表"]["成績相關報表"]["五專免試入學相關報表"]["南區五專聯合免試入學集體報名表"].Click += delegate
            {
            ...
            K12.Presentation.NLDPanels.Student.SelectedSourceChanged += delegate
            {
                ...
                {
                    item1["報表"]["成績相關報表"]["五專免試入學相關報表"]["北中南五專(優先)免試入學集體報名表"].Enable = true;
                }
                else
                {
                    item1["報表"]["成績相關報表"]["五專免試入學相關報表"]["北中南五專(優先)免試入學集體報名表"].Enable = false;
            ...
            K12.Presentation.NLDPanels.Student.SelectedSourceChanged += delegate
            {
                ...
                {
                    item1["報表"]["成績相關報表"]["五專免試入學相關報表"]["南區五專聯合免試入學集體報名表"].Enable = true;
                }
                else
                {
                    item1["報表"]["成績相關報表"]["五專免試入學相關報表"]["南區五專聯合免試入學集體報名表"].Enable = false;
            ...
            permission.Add(new RibbonFeature(Permissions.南區五專聯合免試入學集體報名表, "南區五專聯合免試入學集體報名表"));
        }
    }
}
